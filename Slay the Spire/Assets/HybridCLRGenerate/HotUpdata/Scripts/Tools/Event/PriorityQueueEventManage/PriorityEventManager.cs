...
namespace GameFramework
{
    ...
    /// </summary>
    public partial class PriorityEventManager : IPriorityEventManager
    {
        // ---------- 通道实例 ----------

        // 同步事件通道
        private EventChannel<GameEventHandler<GameEventArgs>> syncChannel = new();

        // 异步事件通道
        private EventChannel<GameEventHandlerAsync<GameEventArgs>> asyncChannel = new();
        ...
        public virtual void Subscribe<EventName>(GameEventHandler<GameEventArgs> handler, int priority = 0)
        {
        ...
        public virtual void SubscribeAsync<EventName>(GameEventHandlerAsync<GameEventArgs> handler, int priority = 0)
        {
            if (handler == null) throw new ArgumentNullException(nameof(handler));
